...
public class CharSequenceCompiler {
  ...
  public final ClassLoaderImpl classLoader;
  ...
  public CharSequenceCompiler(ClassLoader loader) {
    this(loader, null);
  }
   ...
   *                                       <var>types</var>.
   */
  public synchronized Class<?> compile(final String qualifiedClassName,
    ...
    }
    {
      ...
      Map<String, Class<?>> compiled = compile(classes);
      Class<?> newClass = compiled.get(qualifiedClassName);
      return castable(newClass, types);
    }
  }

  public Class<?> compile(final String qualifiedClassName, final CharSequence javaSource)
    throws CharSequenceCompilerException {
    diagnostics = new DiagnosticCollector<>();

    Map<String, CharSequence> classes = new HashMap<>(1);
    classes.put(qualifiedClassName, javaSource);
    return compile(classes).get(qualifiedClassName);
  }
  ...
  public synchronized Map<String, Class<?>> compile(final Map<String, CharSequence> classes)
    ...
    final CompilationTask task = compiler.getTask(
      null, javaFileManager, diagnostics, options, null, sources);
    final Boolean result = task.call();
    if (result == null || !result) {
      String err = diagnostics.getDiagnostics().stream().map(Object::toString).collect(Collectors.joining("\n"));
      throw new CharSequenceCompilerException(err, classes.keySet(), diagnostics);
    }
    try {
